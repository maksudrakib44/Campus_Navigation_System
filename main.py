...
def dijkstra_second_best_path(graph, start, goal):
    queue = [(0, [start])]
    paths = []
...
def HighlightPaths(shortest_path, alternative_path):
    map.delete("shortest_path")
    map.delete("alternative_path")

    if shortest_path:
        cost, path = shortest_path
        for i in range(len(path) - 1):
            map.create_line(
                path[i], path[i + 1], fill="blue", width=3, tags="shortest_path"
            )

    if alternative_path:
        cost, path = alternative_path
        for i in range(len(path) - 1):
            map.create_line(
                path[i], path[i + 1], fill="red", width=2, dash=(5, 2), tags="alternative_path"
            )


def UpdateMap(start_name, end_name):
    start = GetCoordsFromName(start_name)
    end = GetCoordsFromName(end_name)

    shortest = dijkstra_shortest_path(graph, start, end)
    alternative = dijkstra_second_best_path(graph, start, end)

    HighlightPaths(shortest, alternative)

    # Create distance labels only when paths are found
    global distance_label, alt_distance_label

    if shortest[0] != float('inf'):
        distance_label = ttk.Label(left, text=f"Shortest path: {shortest[0]:.2f} units",
                                   font=cusfont, background="black", foreground="white")
        distance_label.place(relx=0.5, rely=0.1, anchor="center")
    else:
        distance_label = ttk.Label(left, text="No path found",
                                   font=cusfont, background="black", foreground="white")
        distance_label.place(relx=0.5, rely=0.1, anchor="center")

    if alternative and alternative[0] is not None:
        alt_distance_label = ttk.Label(left, text=f"Alternative path: {alternative[0]:.2f} units",
                                       font=cusfont, background="black", foreground="white")
        alt_distance_label.place(relx=0.5, rely=0.15, anchor="center")
...
# Initialize distance labels (will be created when paths are shown)
distance_label = None
alt_distance_label = None
...
def dropdown():
    ...
    options = [marker.name for marker in locationMarkersList]
    sel = tk.StringVar()
    sel0 = tk.StringVar()
...
def resetui():
    ...
    if distance_label:
        ...
        distance_label = None
    if alt_distance_label:
        alt_distance_label.destroy()
        alt_distance_label = None
